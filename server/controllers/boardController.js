...
var deleteMedia = function(req,res){
	...
	for(i in result.Medias){
	    if (result.Medias[i].MediaID==req.body.media) {
		key=result.Medias[i]._id;
	    }
	}
	result.Medias.id(key).remove();
	result.save(function(err){
...
var myBoards=function(req,res){
    ...
    if(req.session.user) {
	board.find({$or:[{"Invitees.UserID":req.session.user._id},{OwnerId:req.session.user._id}],IsDeleted:{'$ne':1}},{Medias:0}).skip(offset).limit(limit).exec(function(err,result){	    
	    if (err) {
			res.json({"code":"404","message":err})
	    }
	    else{
			board.find({$or:[{"Invitees.UserID":req.session.user._id},{OwnerId:req.session.user._id}],IsDeleted:{'$ne':1}},{Medias:0},function(err,data){
				if (err) {
				   res.json({"code":"404","message":err})
				}else{
					console.log('--------------------------');
