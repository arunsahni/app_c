...
var showMoreMedia = function(req,res) {
	if(req.session.user){
		console.log("Session is set!");
		console.log(JSON.stringify(req.session.user));
		if( req.session.user._id != undefined ){
	...
	var fields = {};
		...
		userMedia_userIdmodel.find({},{"_id": 1}).exec(function (err,userMedias) {
			if (err) {
				...
				userMedia_userIdmodel.find({}).skip(offset).limit(pageLimit).exec(function (err,result) { // Save
					if (err) {
						res.json({"status":"error","message":err});
						return;
					}
						...
						if (result.length) {
							if (result.length < pageLimit) {
								...
								media.find(conditions,fields).sort(sortObj).limit(tobefetchTotalRecord).exec(function (err,resultfromMedia) { // Save
										if (err) {
											res.json({"status":"error","message":err});
											//return;
										}else{
											if (resultfromMedia.length) {
												...
												for( var key in resultfromMedia ){
													var obj = {};
											...
											res.json({"status":"200","result":result,"resultfrommediaId": resultfrommediaId});
										}
								});
							...
							if (mediaId.length > 0) {
								conditions._id = {$nin : mediaId};
							}
							
							media.find(conditions,fields).skip(mediaOffset).sort(sortObj).limit(pageLimit).exec(function (err,result) { // Save
								if (err) {
									res.json({"status":"error","message":err});
									//return;
								}else{
									console.log("userMediaIds",userMediaIds);
									console.log("userMediaIds count: ",userMediaIds.length);
									if (result.length) {
									...
									res.json({"status":"200","mediaOffset": mediaOffset, "result":resultfromMediaFinal});
								}
							});
						}
					}
...
var search_v_8_temp = function( req , res ){
    ...
    */
    ...
    if( mTypeArr.length ){
        //searchObj.query={"GroupTags.GroupTagID":groupTagID,"Status":1,"IsPrivate":{$ne:1},$or:mTypeArr,"IsDeleted":0};
        searchObj.query={"Status":1,"IsPrivate":{$ne:1},"IsDeleted":0,$and : [{$or:mTypeArr}]};
        //searchObj.query = Object.assign(searchObj.query,{$and : [{$or:mTypeArr}]});
        //searchObj.query["$and"] = mTypeArr;
    }
    else{
        //searchObj.query={"GroupTags.GroupTagID":groupTagID,"Status":1,"IsPrivate":{$ne:1},"IsDeleted":0};	
    ...
    if(mediaTitle){
        //searchObj.query={"GroupTags.GroupTagID":groupTagID,"Status":1,"IsPrivate":{$ne:1},"Title":new RegExp(mediaTitle, 'i'),"MediaType":mediaType,"IsDeleted":0};
        //searchObj.query={"GroupTags.GroupTagID":groupTagID,"Status":1,"IsPrivate":{$ne:1},"Title":new RegExp(mediaTitle, 'i'),$or:mTypeArr,"IsDeleted":0};
        searchObj.query={"Status":1,"IsPrivate":{$ne:1},"Title":new RegExp(mediaTitle, 'i'),"IsDeleted":0,$and : [{$or:mTypeArr}]};
        //searchObj.query = Object.assign(searchObj.query,{$and : [{$or:mTypeArr}]});
    }
...
//exports.search_v_8_temp = search_v_8_revised_4;
