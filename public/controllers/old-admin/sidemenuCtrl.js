...
module.controller('sidemenuCtrl', function ($rootScope,$scope,$http,$timeout,$upload,$location,loginService,$window) {
	...
	*/
	...
	$scope.showFiltered = function(){
		...
		console.log("My params",$scope.filtering_params);
	...
	$http.get('/domains/view').then(function (data, status, headers, config) {			
		if (data.data.code==200){
			...
			console.log("Doamin---------------------",$scope.domain);
		}	
	})
	...
	*/
	...
	$scope.deleteDescriptor = function(media, descriptorId, descriptorTitle){
		if (descriptorId != undefined && descriptorId != null) {
			if (confirm("Do you want to delete that tag?")) {
				/*
				angular.forEach($scope.media_gallery_data.response, function(value,key){
					if(value._id == media._id){
						var promptArr1 = value.Prompt.split(',');
						...
						$scope.media_gallery_data.response.Prompt = promptArr1.join();
						console.log("Prompt---------------",$scope.media_gallery_data.response.Prompt);
					}
				});
				*/
				for(var loop=0; loop < $scope.media_gallery_data.response.length;loop++){
					if($scope.media_gallery_data.response[loop]._id == media._id){
						...
						//$scope.media_gallery_data.response[loop].Prompt = $scope.media_gallery_data.response[loop].Prompt ? $scope.media_gallery_data.response[loop].Prompt : "";
	...
	//$http.get('/groupTags/view').then(function (data, status, headers, config) {			
				...
				//		$scope.gts=data.data.response;
				//		for(key in $scope.gts){
				//			if($scope.gts[key]._id == $scope.addedTag.gt){
				//				console.log("Data to push",$scope.gts[key]);
				//				var length = $scope.Media.GroupTags.length;
				//				$scope.Media.GroupTags.splice(length,$scope.gts[key]);
				//				console.log("After splice",$scope.Media.GroupTags)
				//			}				
				//		}
				//	}
				//})
...
//module.directive('autoCompleteFilter', function($timeout, $http) {
//    return function(scope, iElement, iAttrs) {
//            iElement.autocomplete({
//                source: function(request,response){
//			  console.log("My Text box value------------------->",request.term)
//				//console.log(request);
//				scope.groupta = [];
//				scope.groupt = [];
//				scope.keywordList = [];
...
/*$scope.keywordsSelcted = [];
	...
	$scope.excludeWords = [];
	*/
//$scope.filtering_params.keywordsSelcted = [];
    ...
    return function(scope, iElement, iAttrs) {                        
		iElement.autocomplete({
			source: function(request,response){
				...
				scope.removeAt = function(keyword){
				      scope.addAnotherTag.splice(keyword,1);
				      scope.groupt.splice(0, keyword);
				}
				scope.removeEW = function(keyword){
				      scope.excludeWords.splice(keyword,1);
				      scope.groupt.splice(0, keyword);
				}
...
/* service Code */
//module.service('PreviewService', ['$http' , '$q' , function( $http , $q ){
//	//get requests list
//	return {
