...
*/   
    $scope.addToMediaTray = function(media,event , pageId){
        if(pageId != null){
            if(event){
            ...
            if ($scope.countOfTrayMedia <15) {
                ...
                $compile(test)($scope);
                $('#'+pageId+' #searchList__mediaTray').append(test);
...
_________________________________________________________________________
*/ 
	$scope.sync_discussTray = function(pageId){
            if(pageId != null){
                if ($('#'+pageId+' #searchList__mediaTray').find('.empty-list').is('img')) {
			
		}
		else{
                    var search_tray_items = angular.element('<ul id="discussList__mediaTray">'+$('#'+pageId+' #searchList__mediaTray').html()+'</ul>');
                    search_tray_items.find('li').each(function(){
                        $(this).find('.close_icon').css({'display':'block'});
                        $(this).find('a').attr('onClick','mediaSite.discussTrayClick($(this) , "'+pageId+'")');
                    })
                    $('#'+pageId+' .discuss_trayArea').html(search_tray_items);
		}
...
_________________________________________________________________________
*/ 
	$scope.sync_searchViewTray = function(pageId){
            if(pageId != null){
                if ($('#'+pageId+' #searchList__mediaTray').find('.empty-list').is('img')) {
		}else{
                    var search_tray_items = angular.element('<ul id="searchView-media-tray">'+$('#'+pageId+' #searchList__mediaTray').html()+'</ul>');
                    search_tray_items.find('li').each(function(){
                        $(this).find('.close_icon').css({'display':'block'});
                        $(this).find('a').attr('onClick','montage_ele.gridster_tray($(this) , "'+pageId+'")');
                        $(this).addClass('drag-me');
                    })
                    setTimeout(function(){$('.drag-me').draggable({revert:'invalid',helper: 'clone'})},50);
                    $('#'+pageId+' #search_view .image-selector').html(search_tray_items);
		}
            }
            else{
                if ($('#searchList__mediaTray').find('.empty-list').is('img')) {
...
/*________________________________________________________________________
	...
	_________________________________________________________________________
	*/
	$scope.addtoTray_delCase = function(mediaID , pageId){
            if(pageId != null){
                if ($scope.countOfTrayMedia < 15) {
                    $http.post('/media/viewMedia',{ID:mediaID}).then(function(response){
                        var data = response.data.result[0];
                        $scope.addtoTray_uploadCase(data , pageId);
                    })
		}else{
                    $('#'+pageId+" #media_tray_full_pop").show();
		}
...
_________________________________________________________________________
*/     
	$scope.addtoTray_uploadCase = function(media , pageId){
            if(pageId != null){
                console.log('addtoTray_uploadCase ')
                if ($scope.countOfTrayMedia <15) {
                    $('#'+pageId+' .avatar-widget').css({'display':'block'});
                    $scope.countOfTrayMedia++;
                    var test='';	
                    var func_setID = "setMediaIdd_uploadCase('"+media._id+"' , '"+pageId+"')";
