...
*/
$scope.get_allFSG = function(pageId){
    if(pageId != null){
        $scope.fsgs=[];
        $http.get('/fsg/view').then(function (data, status, headers, config){
            if (data.data.code==200){
                        ...
                        if (userInfo.FSGsArr2 && userInfo.FSGsArr2 != [] && userInfo.FSGsArr2 != undefined) {
                            for (x in userInfo.FSGsArr2){
                                if (x == $scope.countries.Title) {
                                    for (var j = 0; j< $scope.countries.Values.length ; j++ ){
                                        //var fsgsArrVal = '';
                                        var flag = false;
                                        var fsgsArrVal = (userInfo.FSGsArr2[x]).split(',');
                                        for( var y = 0; y < fsgsArrVal.length ; y++ ){
                ...
                setTimeout(function(){
                    ...
                    $scope.setDefaultFilters(pageId);
                },10);
            }
        })
...
_________________________________________________________________________
*/
    $scope.set__defaultValueInSelectedFSGs = function(pageId){
        if(pageId != null){
            $scope.selectedFSGs={};
            var loopFlag = -1;
...
_________________________________________________________________________
*/
    $scope.changeCss = function(id,sg,fsg , pageId){
        if(pageId != null){
            $('#overlay2').show();
            $('#overlayContent2').show();
            console.log('----------changeCss-------------');
            var loopFlag = -1;
            if ($('#'+pageId+' #'+id).hasClass('all')) {
...
_________________________________________________________________________
*/
    $scope.setDefaultFilters=function(pageId){
        if(pageId != null){
            $timeout(function(){
                for (i in $scope.fsgs) {
                                ...
                                for (j in $scope.fsgs[i].Values ){
                                    ...
                                    for(y in fsgArrVal){
                                        if ($scope.fsgs[i].Values[j].valueTitle==fsgArrVal[y]) {
                                            $('#'+pageId+' #'+$scope.fsgs[i].Values[j]._id).prop('checked',true);
                                        }
                                    }
                                }
                            }
                ...
                if ($scope.userInfo.Settings!=[]) {
                    if ($scope.userInfo.Settings[0] != [] && $scope.userInfo.Settings[0] != undefined) {
                        ...
                        if ($scope.userInfo.Settings[0].Prefrence != '' && $scope.userInfo.Settings[0].Prefrence != undefined) {
                            $('#'+pageId+' #'+$scope.userInfo.Settings[0].Prefrence).prop('checked',true);
                        }
                        ...
                        if ($scope.userInfo.Settings[0].MediaType != '' && $scope.userInfo.Settings[0].MediaType != undefined && $scope.userInfo.Settings[0].MediaType.length != 0) {
                            for (i in $scope.userInfo.Settings[0].MediaType) {
                                ...
                                $('#'+pageId+' #'+$scope.userInfo.Settings[0].MediaType[i]).prop('checked',true);
                            }
                            $('#'+pageId+' .select_media').each(function(){
                ...
                $scope.set__defaultValueInSelectedFSGs(pageId);
            },500);
...
*/
    $scope.contentmediaType = [];
    $scope.changeMediaType = function(type , pageId){
        if(pageId != null){
            $('#overlay2').show();
            $('#overlayContent2').show();
            if (!($('#'+pageId+' .select_media[value='+type+']').prop('checked'))) {
                console.log('in-if');
                var num =0;
                $('#'+pageId+' .select_media').each(function(){
                    if($(this).prop('checked') ){
...
_________________________________________________________________________
*/
    $scope.saveDefault= function(title , pageId){
        if(pageId != null){
...
_________________________________________________________________________
*/
    $scope.saveFormat=function(pageId){
        if(pageId != null){
            ...
            fields.MediaType=[];
            if($('#'+pageId+' #radioImage').prop('checked')){
                fields.MediaType.push("radioImage");
            }
            if($('#'+pageId+' #radioLink').prop('checked')){
                fields.MediaType.push("radioLink");
            }
            if($('#'+pageId+' #radioNotes').prop('checked')){
                fields.MediaType.push("radioNotes");
            }
            if($('#'+pageId+' #radioMontage').prop('checked')){
                fields.MediaType.push("radioMontage");
            }
            //console.log(fields);
            $scope.postSettings(fields , pageId);
        }
        else{
            if ($scope.userInfo.Settings!=[] && $scope.userInfo.Settings != undefined) {
                if ($scope.userInfo.Settings[0] != [] && $scope.userInfo.Settings[0] != undefined) {
...
_________________________________________________________________________
*/
$scope.postSettings  = function(fields , pageId){
    if(pageId != null){
...
_________________________________________________________________________
*/
    $scope.clearAllSelected=function(pageId){
        if(pageId != null){
            $('#'+pageId+' .filter_checbox').each(function(){
                $(this).find('.radiobb').prop('checked',false);
            });
...
_________________________________________________________________________
*/
$scope.getCountrySel = function(pageId){
    if(pageId != null){
        $('#overlay2').show();
        $('#overlayContent2').show();
...
_________________________________________________________________________
*/
$scope.addToSelected_country = function(country , pageId){
    if(pageId != null){
        //alert(1);
...
_________________________________________________________________________
*/
$scope.removeSelected_country = function(country , pageId){
    if(pageId != null){
        //alert(1);
...
/**********************************END***********************************/
    ...
    $scope.init__filterCtrl = function(pageId){
        if(pageId != null){
            $scope.get_allFSG(pageId);
        }
        else{
            $scope.get_allFSG();
        }
    }
    $scope.init__filterCtrl($scope.page_id);
    //initial scripts : put always at the end
