...
collabmedia.controllerProvider.register('searchGalleryCtrl',function($scope,$sce,$http,$location,$window,$upload,$stateParams,loginService,$timeout,$compile,$controller,$rootScope,$filter){
    ...
    $scope.filterSub = function(searchFlag , pageId){
        if(pageId != null){
            //alert("called...");
            //$('#overlay2').show();
            ...
            $http.post('/media/searchEngine',queryParams).then(function (data1, status, headers, config) {
                    if (data1.data.status=="success"){
                        ...
                        if ($scope.selectedgt) {
                            ...
                            $('#'+pageId+'.keys #'+$scope.selectedgt).parent().addClass('activeKey');
                        }
                    }
                    else{
                        $('.loader_overlay').hide()
    ...
    $scope.setHeight = function(pageId){
        /*
        if(pageId != null){
            $('#'+pageId+' .gallery-block ul li .hover-overlayer').height($('#'+pageId+' .gallery-block ul li').width());
            $('#'+pageId+' .gallery-block ul li').height($('#'+pageId+' .gallery-block ul li').width());
            $('#'+pageId+' .gallery-block ul li .gallery-pic-bx').height($('#'+pageId+' .gallery-block ul li').width());
            $('#'+pageId+' .gallery-block ul li a').height($('#'+pageId+' .gallery-block ul li').width());
            $('#'+pageId+' .gallery-block ul li img').height($('#'+pageId+' .gallery-block ul li').width());
            //alert("called...");
        }
        else{
    ...
    $scope.arrayOfMedias=[];
    $scope.init__showMorePagination = function(pageId){
        if(pageId != null){
            $scope.per_page = 48;
            $scope.page = 1;
            $scope.arrayOfMedias=[];
    ...
    $scope.show_more = function (pageId) {
        if(pageId != null){
            $('#loading').show();
            $scope.page += 1;
            $scope.filterSub__showMore(pageId);
    ...
    $scope.has_more = function (pageId) {
        if(pageId != null){
            return $scope.more;
        }
        else{
    ...
    $scope.filterSub__showMore = function(searchFlag , pageId){
        if(pageId != null){
            if(!$scope.ownerFSG){
                userFsgs = $scope.selectedFSGs;
            }
            ...
            if(searchFlag == 'search-by-theme'){
                ...
                $('#'+pageId+' #slideTitleHeader li').each(function(){
	...
	$scope.setShowMorePagination = function(response , pageId){
            if(pageId != null){
                for(i in response){
	...
	$scope.parseKeywords__old = function (pageId){
	    if(pageId != null){
                var postData = {};
		postData.keywordsArr = $scope.last_list;
		postData.inputText = $('#'+pageId+' .ansr-box-inr').text();
                    ...
                    if(data.status == 200){
                        ...
                        if( angular.toJson($scope.keywordsArr) != angular.toJson(data.data.response.suggestedArr) ){
                            ...
                            setTimeout(function(){
                                ...
                                mediaSite.initSlider(pageId);
                            },100)

                            setTimeout(function(){
                                $scope.filterSub(pageId);
                            },100);
                        }
                    }
		});
	...
	$scope.parseKeywords = function (pageId){
            if(pageId != null){
                var data = ($('#'+pageId+' #edit55').text().trim().split('Start typing and click on the keywords OR drop any media from the web or your drive.')).join(' ');
                if ( data.trim() != '' ) {
                    var postData = {};
                        ...
                        if(data.status == 200){
                            ...
                            if (data.data.response.newGT.length>0) {
                                ...
                                setTimeout(function(){
                                    ...
                                    $('#'+pageId+' #lmn_slider').scrollLeft($('#'+pageId+' #lmn_slider ul').width());
                                },100);
                            }
...
*/
	$scope.setMediaId = function(media , pageId){
            if(pageId != null){
...
*/
    var fsgFlag = 0;
    $scope.changeOwner =function(fsgFlag , pageId){
        if(pageId != null){
            $('#overlay2').show();
            $('#overlayContent2').show();
            setTimeout(function(){
...
_________________________________________________________________________
*/
$scope.setGT = function(gt , pageId){
    if(pageId != null){
        console.log('setGT()');
	$('#'+pageId+' .keys').removeClass('activeKey');
	console.log($('#'+pageId+' .keys #'+gt));
	$('#'+pageId+' .keys #'+gt).parent().addClass('activeKey');
	$scope.selectedgt = gt;
	$scope.filterSub(pageId);
...
_________________________________________________________________________
*/
$scope.MakeBold = function(key , pageId){
    if(pageId != null){
        //alert(1);
        var _break = /<br\s?\/?>$/g,
        ...
        var html = angular.element('#'+pageId+' #edit55').html().replace(_break , ""),
        dom = $('#'+pageId+' .ansr-box-inr');
        ...
        innerHtml = angular.element('#'+pageId+' #edit55').html();
        //html;
...
/**********************************END***********************************/
	...
	function placeCaretAtEnd (pageId) {
            if(pageId != null){
                //dom = document.getElementById('edit55');
                dom = angular.element('#'+pageId+' #edit55');
		var range, sel;
		dom.focus();
        ...
        $scope.init__searchGalleryCtrl = function(pageId){
            if(pageId != null){
                $timeout(function(){
                    $scope.setHeight(pageId);
                },2000);
                $scope.init__showMorePagination(pageId);
        ...
        $scope.init__searchGalleryCtrl($scope.page_id);
        //initial scripts : put always at the end
});