...
collabmedia.controllerProvider.register('userGalleryCtrl',['$scope','$stateParams','$location','loginService','SgPageService','LSService','$http','$timeout','$filter','$upload','$window','$q', function($scope,$stateParams,$location,loginService,SgPageService,LSService,$http,$timeout,$filter,$upload,$window,$q){
...
queryParams = {page:$scope.page, per_page:this.per_page, mediaType: this.contentmediaType,userFSGs: $scope.selectedFSGs,groupTagID:$scope.current__gtID, keywordsSelcted: this.keywordsSelcted, addAnotherTag: this.addAnotherTag, excludeTag: this.excludeTag};
                ...
                manageSelection : function (data){
                  ...
                  selectAll : function (){
                     if ($scope.SgPageMgmt.medias.length) {
                        for(key in $scope.SgPageMgmt.medias){
                            var mediaId  = $scope.SgPageMgmt.medias[key]._id;
                                        $scope.SgPageMgmt.selectedMedia.push(mediaId);
                                        $('#'+mediaId).addClass('item-selected');
                 ...
                 deSelectAll : function (){
                   // alert($scope.SgPageMgmt.selectedMedia.length);return
                    if ($scope.SgPageMgmt.medias.length) {
                        for(key in $scope.SgPageMgmt.medias){
                            var mediaId  = $scope.SgPageMgmt.medias[key]._id;
                                        $scope.SgPageMgmt.selectedMedia=[];
                        ...
                        alert('Media deselection limit reached!!')
                    }
                },
		...
		openLaunchSetting : function(section){
                ...
                mediaByPage: function(currentPage){
                    ...
                    $scope.SgPageMgmt.filterByPage(currentPage);
                },
                filterByPage : function(currentPage){
                    $scope.arrayOfMedias=[];
                    var queryParams = {};
                    if(!$scope.ownerFSG){
                        userFsgs=$scope.selectedFSGs;
                ...
                openUserGallery : function(){
                    ...
                    $window.open('#/user-gallery/'+this.capsule_id+'/' + this.chapter_id + '/' + this.page_id);
                },
                userGalleryPreview : function(){
                   
                    $http.post('/pages/userGallery',this.page_id).then(function (data1, status, headers, config) {
