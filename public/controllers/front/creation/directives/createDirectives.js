...
collabmedia.compileProvider.directive("lmnSliderAnother", [function () {
    return {
        link: function(scope, element, attr) {
            element.lemmonSlider();
            function getKeys(){
                return scope.SgPageMgmt.addAnotherTag;
            }
            scope.$watch(getKeys, function () {
                console.log('value changed');
                element.lemmonSlider('destroy');
                element.lemmonSlider({'loop':false});
                var setWidth = ($('#another_tag_slider ul li').length)*33 +$('#another_tag_slider ul').width();
                $('#another_tag_slider ul').width(setWidth);
            },true);
...
collabmedia.compileProvider.directive("lmnSliderExclude", [function () {
    return {
        link: function(scope, element, attr) {
            element.lemmonSlider();
            function getKeys(){
                return scope.SgPageMgmt.excludeTag;
            }
            scope.$watch(getKeys, function () {
                console.log('value changed');
                element.lemmonSlider('destroy');
                element.lemmonSlider({'loop':false});
                var setWidth = ($('#exclude_tag_slider ul li').length)*33 +$('#exclude_tag_slider ul').width();
                $('#exclude_tag_slider ul').width(setWidth);
            },true);
    ...
    return function(scope, iElement, iAttrs) {                        
        iElement.autocomplete({
            source: function(request,response){
                ...
                scope.groupta = [];
                scope.groupt = [];
                $http.post('/pages/getallKeywordsByCondition',{startsWith:request.term,keywordType: scope.SgPageMgmt.keywordType}).then(function(data,status,headers,config){
                    if (data.data.code==200){
			...
			if (scope.SgPageMgmt.addAnotherTag) {
			  angular.forEach(scope.groupt, function(value, key) {
				  angular.forEach(scope.SgPageMgmt.addAnotherTag, function(selected, key1) {
			...
			if (scope.SgPageMgmt.excludeTag) {
			  angular.forEach(scope.groupt, function(value, key) {
				  angular.forEach(scope.SgPageMgmt.excludeTag, function(selected, key1) {
					  if(value.value == selected.id) {
						  scope.groupt.splice(key, 1);
					  }
				  });
		    ...
		    if (scope.SgPageMgmt.keywordsSelcted.length < 10) {
		      ...
		      scope.$apply();
		        if ($('#keyword_slider ul').width() >$('#keyword_slider').width()) {
                    ...
                    setTimeout(function(){
                        ...
                        scope.SgPageMgmt.filterByPage('tag1');
                        $('.keys').removeClass('activeKey');
                        $('.keys').removeClass('activeKey');
                        $('.keys').last().addClass('activeKey');
                        $('#keyword_slider').scrollLeft($('#keyword_slider ul').width())
			//$http.post('/pages/filterTailorGallery',{keywordSelected: scope.SgPageMgmt.keywordsSelcted,addAnotherTag: scope.SgPageMgmt.addAnotherTag,excludeTag: scope.SgPageMgmt.excludeTag}).then(function (data1, status, headers, config) {
			//  if (data1.data.status== 200){
			//    console.log("Result",data1.data.result)
			//    
			//  }
			//  else{
			//	 
			...
			//});
		      ...
		      scope.setFlashInstant('<span style="color:red">Limit exceeded ! Maximum 10 tags allowed</span>' , 'success');
		      iElement.val('');
		    }
		     return false;
...
collabmedia.compileProvider.directive('videoUpload', function($q) {
    return function(scope, iElement, iAttrs) {
...
////------------ directive for grouptags drop-down --autocomplete
//collabmedia.compileProvider.directive('autoCompleteTagsCustom', function($timeout,$http) {
//    return function(scope, iElement, iAttrs) {                        
//		iElement.autocomplete({
//			source: function(request,response){
//				//console.log(request);
...
//				scope.keywordList = [];
//				
//				console.log("keywordsSelcted",scope.SgPageMgmt.keywordsSelcted);
//				scope.remove = function(keyword , index){
//				      scope.SgPageMgmt.keywordsSelcted.splice(index,1);
//				      scope.groupt.splice(0, keyword);
//				}
//				scope.removeAt = function(keyword, index){
//				      scope.SgPageMgmt.addAnotherTag.splice(index,1);
//				      scope.groupt.splice(0, keyword);
//				}
//				scope.removeEW = function(keyword, index){
//				      scope.SgPageMgmt.excludeTag.splice(index,1);
//				      scope.groupt.splice(0, keyword);
//				}
//				
//				$http.post('/pages/getallKeywordsByTheme',{startsWith:request.term}).then(function(data,status,headers,config){
//					if (data.data.code==200){
//						scope.gts=data.data.response;
//						for(i in scope.gts){
//							scope.groupta.push({"id":scope.gts[i]._id,"title":scope.gts[i].GroupTagTitle})
...
//							}
//						}
//						if (scope.SgPageMgmt.keywordsSelcted) {
//							angular.forEach(scope.groupt, function(value, key) {
//								console.log("--------------------------",value)
//								angular.forEach(scope.SgPageMgmt.keywordsSelcted, function(selected, key1) {
//									console.log("value----------->", value.value);
//									console.log("Seelet----------->", selected.id);
//									if(value.value == selected.id) {
//										scope.groupt.splice(key, 1);
...
//					scope.SgPageMgmt.addAnotherTag.push({title:ui.item.label,id:ui.item.value,from:'dropDown'});
//					scope.$apply();
//					if ($('#another_tag_slider ul').width() >$('#another_tag_slider').width()) {
//					      $('.prev-slide').fadeIn();
//					      $('.next-slide').fadeIn();
//					}else{
//						$('.prev-slide').fadeOut();
...
//					    $('.keys').removeClass('activeKey');
//					    $('.keys').last().addClass('activeKey');
//					    $('#another_tag_slider').scrollLeft($('#another_tag_slider ul').width())
//					},1000);
//					iElement.val('');
//					return false;
//				} else if (iAttrs.id == 'exclude') {
//					scope.SgPageMgmt.excludeTag.push({title:ui.item.label,id:ui.item.value,from:'dropDown'});
//					scope.$apply();
//					if ($('#keyword_slider ul').width() >$('#keyword_slider').width()) {
//					      $('.prev-slide').fadeIn();
//					      $('.next-slide').fadeIn();
...
collabmedia.compileProvider.directive('autoCompleteTagsAnother', function($timeout,$http) {
    return function(scope, iElement, iAttrs) {                        
        iElement.autocomplete({
            source: function(request,response){
                //console.log(request);
		   ...
		   // console.log('---------------------------------------------------'+ui);return false;
		    ...
		    //scope.tagValue = 'tag2';
                    ...
                    scope.selectedgt = ui.item.value;
                    ////scope.gt_fromDwn.push({title:ui.item.label,id:ui.item.value})
		     if (scope.SgPageMgmt.addAnotherTag.length < 10) {
				  scope.SgPageMgmt.addAnotherTag.push({title:ui.item.label,id:ui.item.value,from:'dropDown'});
			    console.log("scope.SgPageMgmt.addAnotherTag----",scope.SgPageMgmt.addAnotherTag);
			    scope.$apply();
			    //$('#keyword_slider').lemmonSlider('destroy');
			    //setTimeout(function(){
			    //	createModule.initSlider();
				...
				$('#loader').show().css({'opacity':'1'});
				//scope.SgPageMgmt.filterSub('search-by-theme');
				scope.SgPageMgmt.filterByPage('tag2');
				$('.keys').removeClass('activeKey');
				$('.keys').removeClass('activeKey');
				$('.keys').last().addClass('activeKey');
				$('#another_tag_slider').scrollLeft($('#another_tag_slider ul').width())
				//$http.post('/pages/filterTailorGallery',{keywordSelected: scope.SgPageMgmt.keywordsSelcted.id,addAnotherTag: scope.SgPageMgmt.addAnotherTag.id,excludeTag: scope.SgPageMgmt.excludeTag.id}).then(function (data1, status, headers, config) {
				//  if (data1.data.status== 200){
				//    console.log("Result",data1.data.result)
				//  }
				//  else{
        ...
        .data( "uiAutocomplete" )._renderItem = function( ul, item ) {
            var flag = false;
            for(var i= 0; i<scope.SgPageMgmt.addAnotherTag.length; i++){
                    if (scope.SgPageMgmt.addAnotherTag[i].id == item.value) {
                            console.log('here')
                            flag = true;
                    }
            }
...
collabmedia.compileProvider.directive('autoCompleteTagsExclude', function($timeout,$http) {
    return function(scope, iElement, iAttrs) {                        
        iElement.autocomplete({
            source: function(request,response){
                //console.log(request);
		   ...
		   // console.log('---------------------------------------------------'+ui);return false;
		    ...
		    //scope.tagValue = 'tag2';
                    ...
                    scope.selectedgt = ui.item.value;
                    ////scope.gt_fromDwn.push({title:ui.item.label,id:ui.item.value})
		    if (scope.SgPageMgmt.excludeTag.length < 10) {
			scope.SgPageMgmt.excludeTag.push({title:ui.item.label,id:ui.item.value,from:'dropDown'});
			console.log("scope.SgPageMgmt.excludeTag----",scope.SgPageMgmt.excludeTag);
			scope.$apply();
			//$('#keyword_slider').lemmonSlider('destroy');
			//setTimeout(function(){
			//	createModule.initSlider();
			//},100)
			if ($('#exclude_tag_slider ul').width() >$('#exclude_tag_slider').width()) {
				$('.prev-slide').fadeIn();
				$('.next-slide').fadeIn();
			}else{
				$('.prev-slide').fadeOut();
			...
			setTimeout(function(){
			    $('#loader').show().css({'opacity':'1'});
			    //scope.SgPageMgmt.filterSub('search-by-theme');
			    scope.SgPageMgmt.filterByPage('tag3');
			    $('.keys').removeClass('activeKey');
			    $('.keys').removeClass('activeKey');
			    $('.keys').last().addClass('activeKey');
			    $('#exclude_tag_slider').scrollLeft($('#exclude_tag_slider ul').width())
			//    $http.post('/pages/filterTailorGallery',{keywordSelected: scope.SgPageMgmt.keywordsSelcted.id,addAnotherTag: scope.SgPageMgmt.addAnotherTag.id,excludeTag: scope.SgPageMgmt.excludeTag.id}).then(function (data1, status, headers, config) {
			//      if (data1.data.status== 200){
			//	console.log("Result",data1.data.result)
			//	
        ...
        .data( "uiAutocomplete" )._renderItem = function( ul, item ) {
            var flag = false;
            for(var i= 0; i<scope.SgPageMgmt.excludeTag.length; i++){
                    if (scope.SgPageMgmt.excludeTag[i].id == item.value) {
                            console.log('here')
                            flag = true;
                    }
            }
