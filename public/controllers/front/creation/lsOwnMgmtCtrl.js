...
collabmedia.controllerProvider.register('lsOwnMgmtCtrl',['$scope', '$location','$timeout','$upload','loginService','LSService','$stateParams','ChaptersService','LaunchSettingsService','CapsulesService','$http', function($scope, $location, $timeout,$upload, loginService,LSService,$stateParams,ChaptersService,LaunchSettingsService,CapsulesService, $http){
		...
		var app = {
			...
			scrollSection: function(sectionName){
                            ...
                            if (sectionName == 'DE') {
                                ...
                                //$('body,html').scrollTop($('.design_n_exp').offset().top-45);
				$('html, body').animate({scrollTop: $('.design_n_exp').offset().top+15}, 1000);
                                $timeout(function(){
	...
	$scope.LSMgmt = (function(){
		var app = {
			...
			toggleGroups: function(index,chapterDataId){
				console.log("Index", index);
				console.log("chapterDataId", chapterDataId);
			...
			//update group listing $scope variable, so that html view will be sync.
				...
				if (validGroup && $scope.LSGroupMgmt.myFile != undefined) {
					...
					if (data.data.code == 200) {
						...
						console.log("$scope.LSMgmt.currentChapterId",$scope.LSMgmt.currentChapterId);
						$scope.LSMgmt.toggleGroups(index);
						//$scope.LSMgmt.set_chapter_views();
