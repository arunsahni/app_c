...
collabmedia.controllerProvider.register('PagesMgmtCtrl',['$scope','$stateParams','$location','$timeout','$state','$window','loginService','DashPagesService','SgPageService','LSService', function($scope,$stateParams,$location,$timeout,$state,$window,loginService,DashPagesService,SgPageService,LSService){
	...
	$scope.PagesMgmt = (function(){
		var app = {
			...
			getChapterName : function(id){
				this.isAllowed();
				DashPagesService.getChapterName(this.chapter_id)
					.then(function(data){
			...
			getPageLibrary : function(qc){
				...
				DashPagesService.getPageLibrary({perPage:$scope.perPage,pageNo:$scope.pageNo,qc:qc ? qc : 'all'})
				.then(function(data){
			...
			getAll : function(){
				...
				DashPagesService.getAll(this.chapter_id)
				.then(function(data){
			...
			create: function(pageType){
				...
				DashPagesService.create(this.chapter_id , pageType)
				.then(function(data){
			...
			addFromLibrary : function( id ){
				...
				DashPagesService.addFromLibrary( this.chapter_id , id )
				.then(function(data){
			...
			remove : function( id ){
				this.isAllowed();
				DashPagesService.remove(this.chapter_id , id)
				.then(function(data){
			...
			reorder : function(){
				...
				DashPagesService.reorder(this.chapter_id , $scope.PagesMgmt.getReorderedArr())
				.then(function(data){
			...
			updateChapterName : function (){
				...
				DashPagesService.updateChapterName(this.chapter_id , this.page_id , $scope.PagesMgmt.chapterName)
				.then(function(data){
					//$scope.PagesMgmt.getAll();
				});
			...
			getAllFriends: function(){
				DashPagesService.getAllFriends().then(function(data){
                            ...
                            LSService.getChapters(this.capsule_id).then(function(data){
                                if (data.data.code==400) {
                                    ...
                                    if (data.data.chapter_ids.length == 0) {
                                        ...
                                        $location.path('/launch/'+$scope.PagesMgmt.capsule_id +'/' +section);
                                    }
                                }
                            });
			...
			getAllDash : function(){
				...
				DashPagesService.getDashAll(this.chapter_id)
				.then(function(data){
					$scope.PagesMgmt.pages = data.data.results;
					angular.element('#loader').css("display", "none");
				});
			},
			publishNewChanges : function(){
				DashPagesService.publishNewChanges(this.chapter_id)
				.then(function(data){
					if (data.status == 200) {
						$scope.setFlashInstant('Published changes successfully' , 'success');
					}
				});
			},
			revertNewChanges : function() {
				DashPagesService.revertNewChanges(this.chapter_id)
				.then(function(data){
					if (data.data.status == 200) {
						$scope.PagesMgmt.pages = [];
						$scope.PagesMgmt.pages = data.data.results;
						$scope.setFlashInstant('New changes reverted successfully' , 'success');
					}
				});
			},
			preview : function( chapterId , pageId ){
				console.log(chapterId,'-------------------------------------',pageId);
				$window.open('#/page-preview/'+chapterId+'/'+pageId)
			}
