...
collabmedia.controller('grouptagCtrl',function($rootScope,$scope,$http,$timeout,$location,loginService,$window,ngTableParams,$filter){
	...
	*/
	...
	//To get data according to MMT by Arun Sahani
	$scope.searchMMT = function(value){
		...
		for(key in $scope.mmts){
			if($scope.mmts[key]._id==$scope.mmtSq){
				$scope.mts=$scope.mmts[key].MetaTags;				
			}
		}
		var offset_no = ($scope.currentPage-1)*$scope.pageSize;
		$scope.mtSq = undefined;
		var conditions = {'mmtSq': $scope.mmtSq,'mtSq': $scope.mtSq,'searchText': $scope.searchText,'limit':$scope.pageSize,'offset':offset_no};
		var api = '/groupTags/mmt';
		if (value == 0) {
			$scope.mmtSq = undefined;
			conditions = {'mmtSq': $scope.mmtSq,'mtSq': $scope.mtSq,'searchText': $scope.searchText,'limit':$scope.pageSize,'offset':offset_no};
			api = '/groupTags/mmt';
		}
		
		
		$http.post(api, conditions).then(function (data, status, headers, config) {			
	...
	//To get data according to MT by Arun Sahani
	$scope.searchMT = function(){
		...
		if ($scope.mtSq == " " || $scope.mtSq == undefined) {
			$scope.mtSq = undefined;
		}
		
		$http.post('/groupTags/mt',{'searchText': $scope.searchText,'mmtSq': $scope.mmtSq,'mtSq': $scope.mtSq,'limit':$scope.pageSize,'offset':offset_no}).then(function (data, status, headers, config) {			
			if (data.data.code==200){
				$scope.amit=data.data.response;
				$scope.data=data.data.response;
				$scope.gtData=data.data.response;
	...
	//For Text Search by Arun Sahani
	$scope.searchQuery = function(){
		...
		var offset_no = ($scope.currentPage-1)*$scope.pageSize;
		if ($scope.searchText) {
			$http.post('/groupTags/search',{'searchText': $scope.searchText,'mmtSq': $scope.mmtSq,'mtSq': $scope.mtSq,'limit':$scope.pageSize,'offset':offset_no}).then(function (data, status, headers, config) {			
				if (data.data.code==200){
					$scope.amit=data.data.response;
					$scope.data=data.data.response;
					$scope.gtData=data.data.response;
	...
	*/
	...
	$scope.get_groupTags = function(pgNo) {
		...
		var conditions = {'limit':$scope.pageSize,'offset':offset_no};
		var api = '/groupTags/view';
		if ($scope.mmtSq) {
			$scope.mtSq = undefined;
			conditions = {'mmtSq': $scope.mmtSq,'mtSq': $scope.mtSq,'searchText': $scope.searchText,'limit':$scope.pageSize,'offset':offset_no};
			//conditions = {'mmt': $scope.mmtSq,'limit':$scope.pageSize,'offset':offset_no};
			api = '/groupTags/mmt';
		}
		
		if ($scope.mtSq) {
			conditions = {'searchText': $scope.searchText,'mmtSq': $scope.mmtSq,'mtSq': $scope.mtSq,'limit':$scope.pageSize,'offset':offset_no};
			//conditions = {'mt': $scope.mtSq,'limit':$scope.pageSize,'offset':offset_no};
			api = '/groupTags/mt';
		}
		
		if ($scope.searchText) {
			conditions = {'searchText': $scope.searchText,'mmtSq': $scope.mmtSq,'mtSq': $scope.mtSq,'limit':$scope.pageSize,'offset':offset_no};
			//conditions = {'searchText':$scope.searchText,'limit':$scope.pageSize,'offset':offset_no};
			api = '/groupTags/search';
