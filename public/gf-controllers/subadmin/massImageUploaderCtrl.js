...
module.controller('massImageUploaderCtrl', function ($rootScope,$scope,$http,$timeout,$upload,$location,loginService,$window) {
	...
	*/
	...
	$scope.change_page = function(page_no){
		...
		$http.post('/massmediaupload/view/subadmin',$scope.filtering_params).then(function (data, status, headers, config) {			
		...
		if ($scope.howToSend != 1) {
			$scope.upload[index] = $upload.upload({
				...
				if($scope.count==$scope.selectedFiles.length){
					...
					$http.post('/massmediaupload/view/subadmin',offsetlength).then(function (data, status, headers, config) {			
							if (data.data.code==200){
								$scope.amit=data.data;
								$scope.current_page=1;
								$scope.loaded=true;
