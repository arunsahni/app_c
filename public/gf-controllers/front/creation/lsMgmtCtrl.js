...
collabmedia.controllerProvider.register('lsMgmtCtrl',['$scope', '$location','$timeout','loginService','LSService','$stateParams','ChaptersService','LaunchSettingsService','CapsulesService','$http','$window', function($scope, $location, $timeout, loginService,LSService,$stateParams,ChaptersService,LaunchSettingsService,CapsulesService, $http, $window){
			...
			//update group listing $scope variable, so that html view will be sync.
				...
				if (!invalid && $scope.LSGroupMgmt.myFile != undefined) {
					...
					if (data.data.code == 200) {
						...
						$scope.LSGroupMgmt.toggleGroups(index);
						$scope.LSMgmt.getAllGroups(1);
